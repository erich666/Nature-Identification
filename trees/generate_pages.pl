...
$cgiUrl = "http://www.realtimerendering.com/domaincgi/";
...
sub USAGE {
	print STDERR "usage: generate_pages.pl [prefix]\n" ;
	exit 1 ;
}
...
sub READCONTENTS {
	...
	printf HTMLFILE "<hr>\n<form action=\"$cgiUrl$outputperl\" method=\"POST\" name=\"$singularTitle\">\n";

	printf HTMLFILE "Check the characteristics that you see (you can always hit \"Back\" and change these later).\n";

	printf HTMLQUIZFILE "<form action=\"$cgiUrl$outputperl\" method=\"POST\" name=\"$singularTitle\">\n";
...
<P>
<INPUT type="submit" value="     Create a Quiz     " align="middle"> or you can clear your choices by hitting <input type="reset" name="mybutton" value="Reset">
<P>
...
Don't check any if you want to be tested on all flowers.
Otherwise, check the ones you want:
<P>
(END HTML SECTION3)
	...
	for ( $i = 1 ; $i <= $nf ; $i++ ) {
		...
		chop $dataname ;
		$quizdataname = $dataname ;
		$dataname =~ s/'/\\'/g;	# quote apostrophe
		printf PERLFILE "\'%s\'", $dataname;
		if ( $i < $nf ) { printf PERLFILE "," } ;
		printf PERLFILE "\n" ;
		printf HTMLQUIZFILE "&nbsp;&nbsp;<INPUT type=\"checkbox\" name=\"o\" value=\"$i\"> $quizdataname<BR>\n";
	}
	printf PERLFILE "\);\n\n" ;
